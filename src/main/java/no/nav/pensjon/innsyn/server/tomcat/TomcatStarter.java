...
public class TomcatStarter {
    ...
    private static void prepareContext(Tomcat server) {
        StandardContext context = addWebapp(server);
    ...
    private static StandardContext addWebapp(Tomcat server) {
        try {
            return (StandardContext) server.addWebapp("", absolutePath(WEBAPP_RELATIVE_PATH));
        } catch (ServletException e) {
            throw new AppServerException("Failed to add webapp", e);
        }
    }
