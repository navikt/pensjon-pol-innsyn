 ...
 */
class SheetFiller<T> internal constructor(private val support: EntitySupport<T>) : SheetPopulator{
    private val sheetName = support.entityDescriptor.entityName
    private val columns = support.entityDescriptor.propertyNames
    override fun populateSheet(workbook: Workbook) {
    ...
    private fun createRows(sheet: Sheet, dateCellStyle: CellStyle) {
        support.entityGetter.entities.forEachIndexed { i, e ->
            support.rowFiller(RowFiller(sheet, dateCellStyle, i + 1), e)
        }
    }
